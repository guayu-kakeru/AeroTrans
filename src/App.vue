...
import {
  ...
  hasRequiredModifier,
} from "./lib/shortcutValidation";
import { findVocabularyMatch } from "./lib/vocabularyMatch";
import type {
  TranslationProvider,
  TranslationResult,
  VocabularyItem,
...
const freeModelPresets = [
  ...
  {
    name: "DeepSeek 官方接口（需 Key）",
    baseUrl: "https://api.deepseek.com/v1/chat/completions",
    model: "deepseek-chat",
  },
  {
    name: "Kimi 官方接口（需 Key）",
...
const currentWord = computed(() => {
  if (
    queueState.value.index < 0 ||
    queueState.value.index >= queueState.value.items.length
  )
    return null;
  return queueState.value.items[queueState.value.index];
});

const currentResultVocabularyItem = computed(() =>
  findVocabularyMatch(
    vocabulary.value,
    inputText.value,
    result.value?.translation ?? null,
  ),
);
...
function isLocalAiConfig(): boolean {
  ...
  return localKeywords.some(
    (keyword) => baseUrl.includes(keyword) || model.includes(keyword),
  );
}

function allowsAnonymousApiEndpoint(): boolean {
  const baseUrl = apiConfig.value.baseUrl.trim().toLowerCase();
  return baseUrl.includes("text.pollinations.ai");
}

function ensureMemoryGenerationAllowed(): boolean {
  if (runningInBrowser.value) {
    settingsMessage.value =
      "浏览器模式不支持联想记忆生成。请在桌面版中配置 API 后使用。";
    return false;
  }

  if (!hasApiKey.value && !allowsAnonymousApiEndpoint()) {
    settingsMessage.value =
      "未检测到 API Key。请先在设置中配置 API Key 后再生成联想记忆。";
    return false;
  }

  if (isLocalAiConfig()) {
    settingsMessage.value =
      "检测到本地模型接口。联想记忆仅支持已配置 API Key 的云端 OpenAI 兼容模型。";
    return false;
  }
...
function buildBrowserTranslation(
  text: string,
...
async function withCurrentWindow<T>(
  runner: (win: import("@tauri-apps/api/window").Window) => Promise<T>,
): Promise<T | null> {
...
async function refreshVocabularyAfterSilentCollect() {
  if (runningInBrowser.value) return;
  if (settings.value.collectionMode !== "silent_all") return;
  await refreshVocabulary();
}
...
async function translateNow() {
  ...
  try {
    ...
    await refreshVocabularyAfterSilentCollect();
  } catch (err) {
...
async function addCurrentToVocabulary(starred = true) {
  ...
  const normalizedTerm = inputText.value.trim();
  const normalizedTranslation = result.value.translation.trim();
  if (
    findVocabularyMatch(vocabulary.value, normalizedTerm, normalizedTranslation)
  ) {
    settingsMessage.value = "当前词条已在单词本中";
    return;
  }
  ...
  if (runningInBrowser.value) {
    ...
    localStorage.setItem(
      "aerotrans_vocabulary",
      JSON.stringify(vocabulary.value),
    );
    queueState.value = createReviewQueue(vocabulary.value);
    return;
  }
...
async function generateMemoryForWord(id: number, regenerate = false) {
  ...
  try {
    const memory = regenerate
      ? await tauriService.regenerateMemory(id)
      : await tauriService.generateMemory(id);
    updateMemoryInCaches(id, memory);
    settingsMessage.value = regenerate
      ? "联想记忆已重新生成"
      : "联想记忆已生成";
  } catch (err) {
...
async function saveAllSettings() {
  ...
  try {
    if (
      hasDuplicateShortcut(
        settings.value.spotlightShortcut,
        settings.value.selectionShortcut,
      )
    ) {
      shortcutStatusMessage.value = "Spotlight 与划词快捷键不能相同";
      return;
    }

    if (!runningInBrowser.value) {
      const spotlightConflict = await tauriService.checkShortcutConflict(
        settings.value.spotlightShortcut,
      );
      if (spotlightConflict.conflict) {
        shortcutStatusMessage.value = `Spotlight 快捷键冲突：${spotlightConflict.reason ?? "系统已占用"}`;
        return;
      }

      const selectionConflict = await tauriService.checkShortcutConflict(
        settings.value.selectionShortcut,
      );
      if (selectionConflict.conflict) {
        shortcutStatusMessage.value = `划词快捷键冲突：${selectionConflict.reason ?? "系统已占用"}`;
        return;
      }
    }
...
async function applyCapturedShortcut(value: string) {
  ...
  try {
    if (!hasRequiredModifier(value)) {
      shortcutStatusMessage.value =
        "快捷键至少包含一个修饰键（Ctrl/Alt/Shift/Meta）";
      return;
    }
    ...
    const anotherValue =
      target === "spotlight"
        ? settings.value.selectionShortcut
        : settings.value.spotlightShortcut;
    if (hasDuplicateShortcut(value, anotherValue)) {
      shortcutStatusMessage.value = "Spotlight 与划词快捷键不能相同";
      return;
    }

    if (!runningInBrowser.value) {
      const conflict = await tauriService.checkShortcutConflict(value);
      if (conflict.conflict) {
        shortcutStatusMessage.value = `快捷键冲突：${conflict.reason ?? "系统已占用"}`;
        return;
      }
    }
...
async function updateCompanion() {
  if (
    !canCompanionPoll(
      settings.value.companionEnabled,
      companionPaused.value,
      translatingCompanion.value,
      runningInBrowser.value,
    )
  ) {
    return;
  }
  ...
  if (!shouldTranslateClipboardText(normalized)) return;
...
async function handleSelectionTranslateTriggered(payloadText?: string | null) {
  ...
  const text =
    typeof payloadText === "string"
      ? payloadText
      : await tauriService.readClipboardText();
  const normalized = (text ?? "").replace(/\s+/g, " ").trim();
  if (!normalized) {
    settingsMessage.value =
      "未读取到选中文本。请先选中文本，再按一次划词快捷键。";
    return;
  }
...
onMounted(async () => {
  ...
  if (!runningInBrowser.value && windowLabel.value === "main") {
    ...
    selectionHotkeyUnlisten = await listen<string>(
      "selection_translate_triggered",
      async (event) => {
...
<template>
  <div
    class="min-h-screen p-3 text-ink"
    :style="appShellStyle"
    @mousedown.left="handleGlobalMouseDown"
  >
    <template v-if="isSpotlightWindow">
      <div
        class="mx-auto mt-2 w-full max-w-[520px] rounded-2xl border border-slate-200 bg-white/86 p-3 shadow-float"
      >
        <div class="mb-2 flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2">
            <select
              v-model="settings.translationProvider"
    ...
    <template v-else>
      <div
        class="mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-white/82 shadow-float"
      >
        <header
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <span
              class="rounded-md bg-teal-50 px-2 py-1 text-xs text-teal-700"
              >{{ status }}</span
            >
            <button
            ...
            <button
              class="h-7 w-7 rounded-md bg-rose-500 text-white"
              @click="closeWindow"
            >
              ×
            </button>
        ...
        <main class="p-4">
            ...
            <section class="rounded-xl border border-slate-200 bg-card/90 p-3">
              <div class="mb-2 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    class="rounded-md bg-slate-900 px-2 py-1 text-xs text-white"
                    @click="toggleReadingCompactMode"
                  >
                    返回完整模式
                  </button>
                </div>
                ...
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      :disabled="
                        !result || Boolean(currentResultVocabularyItem)
                      "
                      :title="
                        currentResultVocabularyItem
                          ? '已收藏到单词本'
                          : '收藏到单词本'
                      "
                      @click="addCurrentToVocabulary(true)"
                    >
                      {{ currentResultVocabularyItem ? "★ 已收藏" : "☆ 收藏" }}
                    </button>
                    <button
                      class="rounded-md border border-slate-200 bg-white px-2 py-1 text-xs text-slate-700"
                      :disabled="speaking"
          ...
          <template v-else>
            ...
            <section
              v-if="activeTab === 'spotlight'"
              class="rounded-xl border border-slate-200 bg-card/90 p-4"
            >
              <div
              ...
              <div class="grid gap-3">
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <button
                    class="rounded-lg bg-warm px-4 py-2 text-white"
                    :disabled="!result"
                    @click="addCurrentToVocabulary(true)"
                  >
                    加入单词本
              ...
              <p v-if="errorMessage" class="mt-2 text-sm text-rose-600">
                {{ errorMessage }}
              </p>
              ...
              >
                <div class="mb-2 text-xs text-slate-500">
                  方向：{{ result.detectedDirection }} · 来源：{{
                    sourceLabels[result.source]
                  }}
                  <span v-if="result.source !== settings.translationProvider"
                    >（已自动回退）</span
                  >
                </div>
                <div class="flex items-center justify-between gap-2">
                ...
                <div class="mt-3 flex flex-wrap gap-2">
                  ...
                  >
                    <span>{{ item }}</span>
                    <button
                      class="rounded bg-slate-100 px-1 py-0.5 text-[10px]"
                      :disabled="speaking"
            ...
            >
              <div class="mb-4 flex items-center justify-between gap-2">
                ...
                <div class="flex flex-wrap items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm">
                    <input
                      v-model="settings.companionEnabled"
                      type="checkbox"
                      class="h-4 w-4"
                    />
                    开启剪贴板监听
                  ...
                  <button
                    ...
                    @click="companionPaused = !companionPaused"
                  >
                    {{ companionToggleLabel(companionPaused) }}
                  </button>
                  <button
              ...
              <div
                v-if="showClipboardHelp"
                class="mb-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600"
              >
                该功能会按节流策略读取剪贴板中的纯文本，并自动翻译短词/短句。为减少系统隐私提示，建议仅在阅读场景中开启。
              </div>

              <div
                class="rounded-xl border border-slate-200 bg-gradient-to-br from-cyan-50 to-amber-50 p-4"
              >
                <p class="mb-2 text-xs uppercase tracking-wide text-slate-500">
                  Companion Preview
                </p>
                <p class="text-sm text-slate-600">
                  {{ companionRawText || "复制文本后自动刷新（Tauri 模式）" }}
                </p>
                <p class="mt-2 text-base">
                  {{ companionResult?.translation || "暂无翻译结果" }}
                </p>
                <p v-if="companionPaused" class="mt-2 text-xs text-amber-700">
                  已暂停自动监听，你仍可使用划词快捷键触发翻译。
                </p>
                <p
                  v-if="companionResult?.phonetics?.length"
                  class="mt-1 text-xs text-slate-500"
                >
              ...
              <div
                v-if="readingCompactMode"
                class="mt-3 rounded-xl border border-teal-200 bg-teal-50 p-3 text-sm text-teal-900"
              >
              ...
              <div class="mt-4 grid gap-3 md:grid-cols-2">
                <label class="text-sm">
                  ...
                  <input
                    v-model.number="settings.companionOpacity"
                    class="mt-2 w-full"
                    type="range"
                    min="20"
                    max="100"
                  />
                  <p class="mt-1 text-xs text-slate-500">
                    该透明度会作用于整个窗口（包含标题栏和内容）。
                  </p>
                  <p class="mt-1 text-xs text-slate-500">
                    这是可透视到底层页面的透明，不是毛玻璃模糊。
                  </p>
                </label>
                <label class="inline-flex items-center gap-2 text-sm">
                  <input
                    v-model="settings.companionMouseThrough"
                    type="checkbox"
                    class="h-4 w-4"
                  />
            ...
            >
              ...
              <div v-if="vocabularyViewMode === 'review'">
                ...
                <div v-else class="grid gap-3">
                  ...
                  >
                    <div class="mb-2 text-xs text-slate-500">
                      第 {{ queueState.index + 1 }} /
                      {{ queueState.items.length }} 张
                    </div>
                    <h3 class="text-2xl font-semibold">
                      {{ currentWord.term }}
                    </h3>
                    <p class="mt-2 text-xs text-slate-500">
                      复习模式默认不显示释义，先回忆后再看列表核对。
                    </p>
                  </div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    ...
                    <button
                      class="rounded-lg bg-cyan-600 px-3 py-2 text-sm text-white"
                      :disabled="memoryLoading"
                      @click="generateMemory(false)"
                    >
                      {{ memoryLoading ? "生成中..." : "生成联想记忆" }}
                    </button>
                    <button
                      class="rounded-lg bg-amber-500 px-3 py-2 text-sm text-white"
                      :disabled="memoryLoading"
                      @click="generateMemory(true)"
                    >
                      重新生成
                    </button>
                  </div>
              ...
              <div v-else class="grid gap-2">
                <p class="text-xs text-slate-500">
                  提示：列表模式也支持联想记忆；若使用本地模型或未配置 API
                  Key，会提示先完成 API 配置。
                </p>
                <div
                  ...
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <div class="flex flex-col items-end gap-2">
                      <button
                        ...
                        :disabled="memoryLoading"
                        @click="
                          generateMemoryForWord(item.id, Boolean(item.aiMemory))
                        "
                      >
                        {{
                          listMemoryLoadingId === item.id
                            ? "生成中..."
                            : item.aiMemory
                              ? "重新生成联想"
                              : "生成联想记忆"
                      ...
                      <button
                        class="rounded-md bg-rose-500 px-2 py-1 text-xs text-white"
                        @click="deleteVocabularyById(item.id)"
                      >
                        删除
                      </button>
                    </div>
            ...
            <section v-if="activeTab === 'settings'" class="grid gap-4">
              <div class="rounded-xl border border-slate-200 bg-card/90 p-4">
                ...
                <div class="grid gap-3 md:grid-cols-2">
                  <div class="text-sm">
                    ...
                    <div class="mt-1 flex gap-2">
                      ...
                      />
                      <button
                        class="rounded-lg bg-slate-200 px-3 py-2 text-xs"
                        @click="startShortcutCapture('spotlight')"
                      >
                        {{
                          capturingShortcut === "spotlight"
                            ? "按键中..."
                            : "录制"
                        }}
                      </button>
                  ...
                  <label class="text-sm">
                    收录策略
                    <select
                      v-model="settings.collectionMode"
                      class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2"
                    >
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                  <button
                    class="rounded-lg bg-ink px-4 py-2 text-sm text-white"
                ...
                <p class="mt-2 text-xs text-slate-500">
                  划词快捷键默认
                  Alt+Shift+L。先选中文本，按一次快捷键即可自动粘贴到翻译页并翻译。
                </p>
              </div>
              ...
              <div class="rounded-xl border border-slate-200 bg-card/90 p-4">
                ...
                <div class="grid gap-3 md:grid-cols-2">
                  ...
                  >
                    API Key 不会写入 SQLite，使用系统凭据管理器（Windows
                ...
                <div class="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <span class="text-sm text-slate-500"
                    >API Key 状态：{{ hasApiKey ? "已配置" : "未配置" }}</span
                  >
                </div>
              </div>

              <p v-if="settingsMessage" class="text-sm text-teal-700">
                {{ settingsMessage }}
              </p>
            </section>
          </template>
